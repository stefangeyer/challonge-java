...
public interface RetrofitParticipantHandler {
    ...
    @GET("v1/tournaments/{tournament}/participants.json")
    Call<List<Participant>> getParticipants(@Path("tournament") String tournamentName);

    @GET("v1/tournaments/{tournament}/participants/{participant_id}.json")
    ...
    @POST("v1/tournaments/{tournament}/participants.json")
    ...
    @PUT("v1/tournaments/{tournament}/participants/{participant_id}.json")
    Call<Participant> updateParticipant(@Path("tournament") String tournamentName,
                                        @Path("participant_id") int participantId,
                                        @Body ParticipantQuery participant);

    @POST("v1/tournaments/{tournament}/participants/{participant_id}/check_in.json")
    Call<Participant> checkInParticipant(@Path("tournament") String tournamentName,
                                         @Path("participant_id") int participantId);

    @POST("v1/tournaments/{tournament}/participants/{participant_id}/undo_check_in.json")
