...
class SimpleAttachmentService(private val restClient: AttachmentRestClient) : AttachmentService {
    ...
    override fun getAttachments(match: Match): List<Attachment> =
            this.restClient.getAttachments(match.tournamentId.toString(), match.id).map { aw -> aw.match_attachment }
    ...
    override fun getAttachment(match: Match, attachmentId: Long): Attachment =
            this.restClient.getAttachment(match.tournamentId.toString(), match.id, attachmentId).match_attachment
    ...
    override fun createAttachment(match: Match, data: AttachmentQuery): Attachment {
        ...
        return this.restClient.createAttachment(match.tournamentId.toString(), match.id, data).match_attachment
    }
    ...
    override fun updateAttachment(match: Match, attachment: Attachment, data: AttachmentQuery): Attachment {
        validateAttachmentQuery(data)
        return this.restClient.updateAttachment(match.tournamentId.toString(), match.id, attachment.id, data).match_attachment
    }
    ...
    override fun deleteAttachment(match: Match, attachment: Attachment): Attachment =
            this.restClient.deleteAttachment(match.tournamentId.toString(), match.id, attachment.id).match_attachment
